...
def train_epoch(model, loader, opt, loss_fn, epoch):
    ...
    # Accumulate predictions and labels for batch metrics
    all_logits = []
    all_labels = []
    batch_losses = []
    ...
    for batch_idx, g in enumerate(pbar):
        ...
        all_labels.append((g.y_node > 0.5).long())
    ...
    for metric in metrics.values():
        metric.update(all_logits, all_labels)
